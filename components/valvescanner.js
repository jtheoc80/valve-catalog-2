  ...
  const processImage = async (imageData) => {
    try {
      ...
      if (data.matches && data.matches.length > 0) {
        ...
        setError('No matching valves found. Please try again with a clearer image.');
      }
    } catch (err) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {results.map((valve, index) => (
              <div key={index} className="border rounded-lg p-4 hover:shadow-lg transition-shadow">
                ...
                <h4 className="font-medium text-lg">{valve.name}</h4>
                <p className="text-sm text-gray-600 mb-2">{valve.spec}</p>
                <div className="mt-2 text-sm">
                  <span className="font-medium">Match Score: </span>
                  <span className={`font-bold ${
                    valve.matchScore >= 90 ? 'text-green-600' :
                    valve.matchScore >= 70 ? 'text-yellow-600' :
                    'text-red-600'
                  }`}>
                    {valve.matchScore}%
                  </span>
                ...
                {valve.details && (
                  <div className="mt-3 text-sm">
                    ...
                    <ul className="list-disc list-inside text-gray-600">
                      <li>Type: {valve.details.type}</li>
                      <li>Size: {valve.details.size}</li>
                      <li>Material: {valve.details.material}</li>
                      <li>Connections: {valve.details.connections}</li>
                      <li>Pressure Rating: {valve.details.pressure_rating}</li>
                    </ul>
                  </div>
                )}
              </div>
...
export default ValveScanner;import React, { useState, useRef } from 'react';
import { Camera, Upload, Search, X } from 'lucide-react';
